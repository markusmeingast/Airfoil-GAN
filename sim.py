...
def coef(x, y, DEBUG=False):
    ...
    ##### PLACE GEOEMETRY HERE!
    xbnd = (x*50+50).round().astype(int)
    ybnd = (y*50+100).round().astype(int)

    for i in range(len(xbnd)):
        barrier[ybnd[i], xbnd[i]] = True

    #for i in range(30,51):
    #    barrier[80-i,i] = True

    #barrier[height//2, (height//2)-8:(height//2)+8] = True
    ...
    def stream(nN, nS, nE, nW, nNE, nNW, nSE, nSW, barrierFx, barrierFy):
        #global nN, nS, nE, nW, nNE, nNW, nSE, nSW, barrierFx, barrierFy
        ...
        ##### CALCULATE SUM OF FORCES OF ALL BARRIERS
        barrierFx = 0.0
        barrierFy = 0.0
        ...
        for ix in range(height):
            for iy in range(width):
                if barrier[ix, iy]:
                    barrierFx += nE[ix, iy] + nNE[ix, iy] + nSE[ix, iy] - nW[ix, iy] - nNW[ix, iy] - nSW[ix, iy];
                    barrierFy += nN[ix, iy] + nNE[ix, iy] + nNW[ix, iy] - nS[ix, iy] - nSE[ix, iy] - nSW[ix, iy];

        ##### RETURN
        return nN, nS, nE, nW, nNE, nNW, nSE, nSW, barrierFx, barrierFy

    # Collide particles within each cell to redistribute velocities (could be optimized a little more):
    def collide(rho, ux, uy, n0, nN, nS, nE, nW, nNE, nNW, nSE, nSW):
    ...
    for _ in range(frames):
        for step in range(20):
            nN, nS, nE, nW, nNE, nNW, nSE, nSW, barrierFx, barrierFy = stream(nN, nS, nE, nW, nNE, nNW, nSE, nSW, barrierFx, barrierFy)
            rho, ux, uy, n0, nN, nS, nE, nW, nNE, nNW, nSE, nSW = collide(rho, ux, uy, n0, nN, nS, nE, nW, nNE, nNW, nSE, nSW)

    ################################################################################
    # %% CALCULATE BARRIER CENTER
    ################################################################################

    xc = numpy.mean(numpy.argwhere(barrier)[:,1])
    yc = numpy.mean(numpy.argwhere(barrier)[:,0])

    ################################################################################
    # %% PLOT
    ################################################################################
    if DEBUG:
        mp.imshow(curl(ux,uy), origin='lower')
        ...
        barrierImage = mp.imshow(bImageArray, origin='lower', interpolation='none')
        mp.arrow(xc,yc,100*barrierFx, 100*barrierFy, width=1.0, color='red')
        mp.show()

    return barrierFx, barrierFy

'''
################################################################################
# %% ORIG PLOTTING ROUTINE
################################################################################

# Here comes the graphics and animation...
theFig = mp.figure(figsize=(8,3))
fluidImage = mp.imshow(numpy.concatenate((curl(ux, uy), curl(ux, uy)), axis=0), origin='lower', norm=mp.Normalize(-.1,.1),
                                    cmap=mp.get_cmap('jet'), interpolation='none')
        # See http://www.loria.fr/~rougier/teaching/matplotlib/#colormaps for other cmap options
bImageArray = numpy.zeros((height, width, 4), numpy.uint8)    # an RGBA image
bImageArray[barrier,3] = 255 # set alpha=255 only at barrier sites
bImageArray = numpy.concatenate((bImageArray, bImageArray), axis=0)
barrierImage = mp.imshow(bImageArray, origin='lower', interpolation='none')
...
def nextFrame(arg):                            # (arg is the frame number, which we don't need)
    ...
    #mp.savefig(frameName)
    #frameList.write(frameName + '\n')
    for step in range(20):                    # adjust number of steps for smooth animation
        stream()
        collide()
    fluidImage.set_array(numpy.concatenate((curl(ux, uy), curl(ux, uy)), axis=0))
    #fluidImage.set_array(0.5*rho*(ux**2+uy**2))

    return (fluidImage, barrierImage)        # return the figure elements to redraw

animate = ma.FuncAnimation(theFig, nextFrame, interval=1, blit=True)
mp.show()
'''
